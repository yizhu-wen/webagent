...
test("simple shopping page captures interaction data", async ({ page }) => {
  ...
  await expect.poll(() => ({
    ...
    liveIq: downloads.some((download) => /^shopping_recording_live_python_iq_\d{8}_\d{6}\.png$/.test(download.suggestedFilename())),
  ...
  expect(shoppingFileNames).not.toContain("stage4_signal_events_amplitude_change.png");
  expect(shoppingFileNames).not.toContain("stage4_signal_events_phase_change.png");
  expect(shoppingFileNames).not.toContain("micro_doppler_left_band.png");
  expect(shoppingFileNames).not.toContain("micro_doppler_right_band.png");
...
test("travel tourism page captures interaction data", async ({ page }) => {
  ...
  await expect.poll(() => ({
    ...
    liveIq: downloads.some((download) => /^travel_recording_live_python_iq_\d{8}_\d{6}\.png$/.test(download.suggestedFilename())),
  ...
  expect(travelFileNames).not.toContain("stage4_signal_events_amplitude_change.png");
  expect(travelFileNames).not.toContain("stage4_signal_events_phase_change.png");
  expect(travelFileNames).not.toContain("micro_doppler_left_band.png");
  expect(travelFileNames).not.toContain("micro_doppler_right_band.png");
