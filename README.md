...
- Stopping sensing automatically downloads the Python-style
  ...
  together with the received-audio WAV, spectrogram, and the available processed
  figures (Stage-4 amplitude/phase lines, left/right Micro-Doppler heatmaps, and
  the optional MLP timeline). `metadata.json` follows the
...
This Live Micro-Doppler panel is on-screen only. The downloaded Micro-Doppler
figures are the processed offline versions, generated by the Python backend from
the recorded WAV as `micro_doppler_left_band.png` and
`micro_doppler_right_band.png`, alongside the Stage-4 amplitude/phase figures.
