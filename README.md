...
- Stopping sensing automatically downloads the Python-style
  ...
  Live Python IQ chart, and separate snapshots of the live left/right
  Micro-Doppler heatmaps. Stop-time processed Stage-4, Micro-Doppler, and
  optional MLP figures can still be displayed on the page, but are not included
...
For live transport, the backend retains the latest unmodified feature column
from each two-chirp group. With a 12 ms chirp period, this produces one Stage-4
point every 24 ms, or approximately `41.67 Hz`.
...
`<recording-prefix>_live_micro_doppler_right_<timestamp>.png`. It also exports
the combined raw-audio and Stage-4 live chart as
`<recording-prefix>_live_python_iq_<timestamp>.png`.

The offline figures select bins and remove clutter using the complete recording
(global complex-mean removal and top-12 motion bins over the whole session) at
full slow-time resolution, so they are the exact reference for the causal live
heatmap. Realtime mode instead maintains exact accumulated statistics up to the
current chirp and subtracts the accumulated complex mean from the trailing
64-chirp window. Consequently, the first live Doppler column appears about
`3.8 s` after sensing starts with the existing 3-second trim, and its
window-center timestamp has `0.384 s` latency.

The Python backend may still generate and display its processed offline
amplitude, phase, Micro-Doppler, and optional MLP figures after Stop. Those
processed figures are intentionally excluded from automatic downloads.
