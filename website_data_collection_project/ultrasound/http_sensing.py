...
UPLOAD_DIR = Path(os.environ.get("SENSING_UPLOAD_DIR", str(WEBAGENT_DIR / "uploads"))).resolve()
...
# Set SENSING_PUBLIC_MODE=1 when the site is exposed beyond localhost, e.g.
# behind an HTTPS tunnel, to refuse the endpoints that write uploads to disk.
PUBLIC_MODE = os.environ.get("SENSING_PUBLIC_MODE", "").strip().lower() in {"1", "true", "yes", "on"}
...
class SensingRequestMixin:
    ...
    def do_GET(self) -> None:
        ...
        if parsed_path == "/realtime":
            ...
            return
        if parsed_path.startswith("/sensing-uploads/"):
            self._serve_sensing_artifact(parsed_path)
            return
        if parsed_path == "/robots.txt":
            # The site's static lookup is scoped to a website version, so an
            # unversioned /robots.txt would fall through to the 404 page and
            # crawlers would never read the rules.
            self._serve_robots_txt()
            return
        self.do_honey_GET()

    def _serve_robots_txt(self) -> None:
        path = Path(getattr(self, "robots_txt_file", ""))
        if not path.is_file():
    ...
    def do_POST(self) -> None:
        ...
        if parsed_path == "/api/send-wav":
            ...
            return
        if parsed_path in {"/api/upload-recording", "/api/analyze-recording"}:
            # Both write uploads to disk and analysis additionally runs a
            # subprocess over them, so they stay off when the site is reachable
            # from the public internet. The browser still exports the full
            # session archive on its own.
            if PUBLIC_MODE:
                self._send_json(403, {
                    "ok": False,
                    "error": "Server-side recording storage is disabled in public mode",
                })
                return
    ...
    def _static_url_for(self, path: Path) -> str:
        relative = path.resolve().relative_to(UPLOAD_DIR)
        return "/sensing-uploads/" + relative.as_posix()

    def _serve_sensing_artifact(self, request_path: str) -> None:
        relative = unquote(request_path.removeprefix("/sensing-uploads/"))
    ...
    def _handle_realtime_websocket(self) -> None:
        ...
        # The DSP stack in ultrasound/requirements.txt is optional: recording and
        # archive export need only the standard library. Report the missing
        # dependency over the socket instead of letting the import kill this
        # connection thread, which would look like an unexplained disconnect.
        try:
            ...
            self._send_ws_json({
                "type": "error",
                "message": (
                    f"live processing unavailable ({error.name} is not installed). "
                    "Install ultrasound/requirements.txt to enable it."
                ),
            })
            self.wfile.write(encode_ws_frame(b"", opcode=0x8))
            self.wfile.flush()
            print(f"Realtime WebSocket rejected: missing dependency {error.name}")
            return
